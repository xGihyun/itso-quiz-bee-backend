...
func (r *repository) GetResult(ctx context.Context, data GetResultRequest) (Result, error) {
	sql := `
	WITH player_written_scores AS (
		...
		WHERE 
			quiz_questions.quiz_id = ($1) AND player_written_answers.user_id = ($2)
		GROUP BY 
			player_written_answers.user_id
	)
	SELECT 
	...
	rows, err := r.querier.Query(ctx, sql, data.QuizID, data.UserID)
	if err != nil {
		return Result{}, err
	}
